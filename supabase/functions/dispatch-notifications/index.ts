...
async function sendWebFcm(token: string, task: Task, accessToken: string) {
  ...
  const response = await fetch(`https://fcm.googleapis.com/v1/projects/${fcmProjectId}/messages:send`, {
    ...
    body: JSON.stringify({ message: { token, notification: { title: 'Nudgee reminder', body: task.title }, data: { taskId: task.id } } }),
  })
  if (!response.ok) throw new Error(`FCM rejected the message (${response.status}): ${(await response.text()).slice(0, 500)}`)
}
...
async function dispatchTask(client: ReturnType<typeof createClient>, task: Task, accessToken: string | null) {
  ...
  if (devices.length === 0) throw new Error('No active device tokens found for this user.')
  let failed = false
  for (const device of devices) {
    ...
    try {
      if (device.platform !== 'web') throw new Error(`${device.platform} delivery is not implemented yet.`)
      if (!accessToken) throw new Error('FCM access token is unavailable.')
      await sendWebFcm(device.token, task, accessToken)
  ...
  const { count: remainingFailures } = await client.from('notification_deliveries').select('id', { count: 'exact', head: true }).eq('task_id', task.id).in('status', ['pending', 'processing', 'failed'])
