...
create or replace function public.claim_device_token(
  p_platform text, p_token text, p_device_name text default null, p_app_version text default null
)
...
begin
  ...
  where user_id = requester_id and platform = p_platform and token = p_token
  order by created_at desc limit 1;

  if current_token_id is null then
    insert into public.device_tokens (user_id, platform, token, is_active, last_seen_at, device_name, app_version)
    values (requester_id, p_platform, trim(p_token), true, now(), p_device_name, p_app_version);
  else
    update public.device_tokens
    set is_active = true, last_seen_at = now(), device_name = p_device_name,
