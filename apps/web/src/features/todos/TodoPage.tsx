...
export function TodoPage({ user, onSignOut }: TodoPageProps) {
  ...
  return <main className="app-shell"><section className="todo-app" aria-labelledby="app-title">
    ...
    <section className="natural-reminder-card" aria-labelledby="natural-reminder-title"><div className="natural-reminder-heading"><span aria-hidden="true">✦</span><div><h2 id="natural-reminder-title">Tell Nudgee naturally</h2><p>Try “Remind me to call Mum tomorrow at 9am”.</p></div></div><form className="natural-reminder-form" onSubmit={(event) => void handleNaturalLanguageSubmit(event)}><label className="sr-only" htmlFor="natural-reminder">Describe your reminder</label><input id="natural-reminder" value={naturalLanguage} onChange={(event) => { setNaturalLanguage(event.target.value); setMessage('') }} placeholder="What do you need a nudge for?" autoFocus required /><button className="add-button" type="submit" disabled={isLoading || isParsing}>{isParsing ? 'Understanding…' : <>Add with Nudgee <span>→</span></>}</button></form></section>
